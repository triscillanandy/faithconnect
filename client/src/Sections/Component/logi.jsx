  ...
  const fetchComments = async (postId) => {
    ...
    const fetchLikes = async (postId) => {
      ...
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          setIsLiked(data.likes > 0); // Assuming data.likes holds the like count
        } else {
            ...
            {errorMessage ? (
              ...
              posts.map((post) => (
                <PostsComponent
                  ...
                  media={post.media}
                  toggleLike={() => toggleLike(post.id)} // Pass postId here
      isLiked={isLiked}
                />
              ))
            )}
...
function PostsComponent({ userImg, userName, description, media, toggleLike, isLiked  }) {
  const settings = {
    dots: true,
    infinite: true,
    speed: 500,
  ...
  return (
    <div className="mt-8 border rounded-lg p-4 max-w-lg mx-auto bg-white shadow-sm">
      ...
      <img
          src={like}
          alt="Like"
          className={`cursor-pointer w-6 h-6 ${isLiked ? 'text-red-500' : ''}`} // Apply red color if liked
          onClick={toggleLike}
        />
